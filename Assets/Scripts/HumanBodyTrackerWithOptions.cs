...
public class HumanBodyTrackerWithOptions : MonoBehaviour
{
    ...
    void OnHumanBodiesChanged(ARHumanBodiesChangedEventArgs eventArgs)
    {
        ...
        foreach (var humanBody in eventArgs.added)
        {
            ...
            // 수정한부분
            HumanBodyTrackerUI.Instance.humanBodyText.text = $"Test2중입니다(bodyTracker)" + $"{this.gameObject.name} {humanBody.name} Position: {humanBody.transform.position}\n" +
            $"LocalPosition: {humanBody.transform.localPosition}\n" + $"{this.gameObject.ToString()} " + $"   tset 종료";

            HumanBodyTrackerUI.Instance.humanBoneControllerText.text = $"Test2중입니다." + $"{this.gameObject.name} {humanBoneController.name} Position: {humanBoneController.transform.position}\n" +
        ...
        foreach (var humanBody in eventArgs.updated)
        {
            ...
            HumanBodyTrackerUI.Instance.humanBodyText.text = $"Test2중입니다.(WITH options)" + $"{this.gameObject.name} {humanBody.name} Position: {humanBody.transform.position}\n" +
            $"LocalPosition: {humanBody.transform.localPosition}";

            HumanBodyTrackerUI.Instance.humanBoneControllerText.text = $"Test2중입니다.(with options)" + $"{this.gameObject.name} {humanBoneController.name} Position: {humanBoneController.transform.position}\n" +
            $"LocalPosition: {humanBoneController.transform.localPosition}";
        }
